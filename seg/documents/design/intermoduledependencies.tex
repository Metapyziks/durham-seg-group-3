...
\def\TabS#1#2{\small\tabular{C{#1}}\rule[-1.5mm]{0pt}{5mm}\textbf{#2}
    \rule[-2mm]{0pt}{2mm}\endtabular}
\def\TabA#1#2#3{\small\tabular{C{#1}}\rule[-1.5mm]{0pt}{5mm}\textbf{#2}\\
...
\begin{figure}[h!]
    \centering
    \begin{tabular}{c}
    \entity{database}[\TabA{3cm}{Database}{(SQLite Database)}]
    \hspace{5mm}
    \entity{server}[\TabA{3cm}{Server}{(Webserver)}]
    \hspace{5mm}
    \tabular{C{1mm}}
    \pnode(0,2){arrowtop}
    ...
    \endtabular
    \hspace{5mm}
    \entity{model}[\TabA{3cm}{Model}{(Application's functionality)}]
    \hspace{5mm}
    \entity{gui}[\TabA{3cm}{Graphical User Interface}{
        \entity{controller}[\TabA{25mm}{Controller}{(User Input)}] \\[5mm]
        \entity{view}[\TabA{25mm}{View}{(Application Output)}]
    }]
    \end{tabular}
    {\small
