...
\section{Solution Requirements}
	...
	\subsection{Game Mechanics} % 2
		...
		\funcreq % 2.25
			...
			{The system must include a method of ensuring that a user is at
			the location they claim they are when they perform a location
			dependent action.}
			{2}{None}
			{The user makes a request that requires the user to be at the
			location they claim to be.}
			{The server performs some validation to ensure the user is
			where they are claiming to be.}
			{If the request passes the server's validation, the request
			is performed.}
		\nonfuncreq % 2.26
		...
		\nonfuncreq % 2.27
			{Difficulty Curve}
			{There should be an inverse relationship between the ease with which 
			a user can expand their territory (own new caches) and the number
			of caches that they own.}
		\nonfuncreq % 2.28
