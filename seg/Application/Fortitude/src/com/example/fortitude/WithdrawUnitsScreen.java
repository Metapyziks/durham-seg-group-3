...
public class WithdrawUnitsScreen extends Window 
{
	...
	{
		super(R.drawable.withdraw_points_bg);
		me = this;
	...
	private GridLayout createWindowPane()
	{
		...
		unitsSeekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			{
				WithdrawUnitsScreen.getMe().setUnitsToPlaceBox(Integer.toString(progress + 1));
			}
											...
											Fortitude.getFortitude().runOnUiThread(new Runnable() {
												public void run()
												{
													...
													if(foundCache)
													{
														...
														MessageBox.newMsgBox("Successfully Withrew Units!", true);
													}
													else
													{
														new MainScreen();
		...
		buttonGrid.addView(withdrawUnitsButton, withdrawUnitsButtonLayout);
	...
	public static WithdrawUnitsScreen getMe()
	{
		return me;
	}
