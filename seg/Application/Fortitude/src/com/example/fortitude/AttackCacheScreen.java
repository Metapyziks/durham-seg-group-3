...
public class AttackCacheScreen extends Window
{
	...
	{
		super(R.drawable.attack_cache_bg);
		me = this;
	...
	private GridLayout createWindowPane()
	{
		...
		cacheNameFirstLeftSpaceLayout.width = (super.getWindowWidth() / 3) + (super.getWindowWidth() / 5);
		...
		cacheNameTextView.setText(AttackCacheScreen.getStaticTheCache().getCacheName());
		...
		unitsSeekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			{
				AttackCacheScreen.getMe().setUnitsToPlaceBox(Integer.toString(progress + 1));
			}
		...
		LayoutParams attackCacheButtonLayout = new LayoutParams(row1, col2);
			...
			public void whenClicked()
			{
				...
				if(CurrentUser.getMe().getIntBalance() > 0)
				{
					...
					Thread thread = new Thread(new Runnable() {
						public void run()
						{
							while(!ServerRequests.getAttackCacheComplete())
							{
								//wait
							}
							if(ServerRequests.getAttackCacheSuccess())
							{
		...
		sixthRowButtonGrid.addView(attackCacheButton, attackCacheButtonLayout);
	...
	public TextView getUnitsToPlaceBox()
	{
		return unitsToPlaceBox;
	}
